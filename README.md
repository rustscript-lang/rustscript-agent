...
| Session compact route (`POST /api/sessions/{id}/compact`) | Implemented as **real manual compaction** through `AgentService::compact_session`: the RSS `compact.rss` policy decides the pair-preserving prefix and the service executes the planned A2 storage commands (`compaction.start → message.compact → compaction.commit`) inside a bounded auditable maintenance run with an exact-once terminal. Typed answers: 200 with the real compaction id/run id/generation/covered range or a typed skip, 409 for an active run (`run_active_conflict`) or an in-flight compaction (`compaction_in_progress`), 503 for missing durable storage / policy failures / storage errors, 404 for an unknown session. See [plans/2026-08-16_a9-manual-session-compaction.md](plans/2026-08-16_a9-manual-session-compaction.md) |
...
| Harness and approval machinery (A4) | Implemented: `registry/file/patch/approval.rss` + `src/runtime/approval_bridge.rs` with durable exact-once pending approvals; the independent native deny policy (`native_hard_deny`, tool/risk lists) also applies without SQLite. The bounded foreground terminal remains a typed `capability_unavailable` core gap. See [plans/2026-08-15_a4-harness-approvals.md](plans/2026-08-15_a4-harness-approvals.md). |
| Parallel tools and subagents (A6) | **Implemented (native supervisor wiring)**: the serial loop defers approved parallel/task delegation with a typed handoff carrying the verified plan request; the native supervisor (`src/runtime/subagent_supervisor.rs` + `ServiceChildExecutor`) runs the RSS policies (`parallel.rss` / `subagents.rss`), admits REAL child runs through `AgentService` (parent link, isolated session, worker spawned before `subagent.started`), awaits bounded durable terminals (`terminal_pending` waits for retry; parent deadline/stop cancels), persists guarded child links, retries lifecycle events with fixed IDs, rebuilds replay fanout from durable `run.get`/`run.list_children`, preserves opaque JSON input, and supplies stable slot/tool-call IDs for race/fail-fast continuation. It enforces bounded concurrency / ordered slots / race / fail-fast / depth / fanout / parent-cancel propagation / deadline, folds the ordered typed results back into the loop state, and the loop continues reasoning. Delegation is approval-gated via the A4 policy (durable park/resume). The only remaining A6 core gap is the script-internal `task::spawn` surface (the policy cannot itself drive children in RSS) — see [plans/2026-08-15_a6-core-blocker.md](plans/2026-08-15_a6-core-blocker.md). |
...
| Telegram gateway (A8) | **Implemented**: native Bot API transport (https/rustls), deny-by-default account/chat/user allowlists, durable delivery cursors (at-least-once), bounded 429/5xx/401 retries, fail-closed first-boot drain, bounded shutdown drain, and the A5 wiring — `/run` (real admission with a durable run-id echo; bare `/run` is a usage error), `/stop` (typed cancel, parked approvals cancelled immediately), `/status` (durable state), `/new` (session reset), `/compact` (REAL manual session compaction through `AgentService::compact_session` — the `telegram:<user_id>` actor is persisted in the maintenance-run audit input; the reply echoes the real compaction id/generation/range or a typed skip/conflict/error, and an active run is refused), and `/approve`/`/deny` (durable approval resolution gated on the run's durable origin actor — foreign/non-owner/owner-less ids are byte-identical to unknown ids, the actor/reason persisted). See [docs/deployment.md](docs/deployment.md). |
