...
/// for exactly-once dispatch; `Terminal` carries a typed terminal reason.
#[derive(Clone, Debug, PartialEq, Eq)]
pub enum Resolution {
    ...
    /// terminal reason.
    Terminal { approval_id: String, reason: String },
}
...
impl ApprovalBridge {
    ...
    /// Returns the persisted approval id, or a typed error.
    pub fn request_pending(&self, request: &PendingApproval) -> Result<String, ApprovalError> {
    ...
    pub fn resolve(
        ...
        if self.row_count(&result) == 1 {
            ...
            // expired) or the run is no longer resolvable.
            if approve {
